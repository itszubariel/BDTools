...
```json
{
  ...
  "line": 1,
  "message": "$sendMessage - Missing closing bracket ']'"
}
```
...
```json
{
  "function": "$sendMessage",
  "line": 1,
  "message": "$sendMessage - Argument 1 has invalid bracket wrapping: '[hello world]'. Did you accidentally use double brackets?"
}
```
...
**Order Checking:** Parent must be defined before child, checking both line number and position within the line. This allows multiple related functions on the same line:

```bdscript
$jsonParse[{}] $jsonArray[eLog] $jsonPretty[3]  ✅ Correct order
$jsonArray[eLog] $jsonParse[{}]  ❌ Wrong order
```
...
```json
{
  ...
  "line": 1,
  "message": "$userLeaderboard - Argument 1 (Variable name) must be a literal variable name, not a function call. Got '$sum[$getUserVar[wordle];$getUserVar[wordle]]'"
}
```
...
```json
{
  "function": "$sendMessage",
  "line": 1,
  "message": "$sendMessage - Incorrect function use: cannot be used inside $title"
}
```
...
$setUserVar[x;$description[hello]]               // ✅ variable functions can contain anything
```
