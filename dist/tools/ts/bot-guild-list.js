...
function buildSkeletons() {
    const cols = getCols(), count = cols * 4;
...
let allGuilds = [], filteredGuilds = [], currentPage = 1;
const ROWS = 4; // rows per page
async function loadGuilds() {
    const key = document.getElementById("apiKeyInput").value.trim();
    if (key)
        localStorage.setItem("bdtools_apikey", key);
    if (!key) {
...
function buildCard(g) {
    ...
    const online = g.presenceCount != null ? parseInt(g.presenceCount).toLocaleString() : null;
    const shortId = g.id || "—";
...
function renderPage() {
    ...
    const slice = filteredGuilds.slice((currentPage - 1) * pageSize, currentPage * pageSize);
...
function onSearch() {
    ...
    filteredGuilds = allGuilds.filter((g) => (g.name || "").toLowerCase().includes(q) || (g.id || "").includes(q));
    applySort();
    currentPage = 1;
    if (allGuilds.length) {
        hideAll();
