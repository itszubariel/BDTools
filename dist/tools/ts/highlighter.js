...
async function loadFunctions() {
    try {
        ...
        allFns = data
            ...
            .filter(Boolean)
            .sort();
        console.log(`[highlighter] ✅ Loaded ${allFns.length} functions from BDFD API`);
    }
    catch (err) {
        console.error("[highlighter] ❌ Failed to load functions from BDFD API:", err.message);
...
if (saved) {
    ...
    baseItems.forEach((i) => (theme[i.name] = { color: i.color, style: i.style }));
}
const listEl = document.getElementById("functions-list");
let currentFn = null, fnToDelete = null, tempStyle = 0;
/* ── Render cards ── */
function renderList() {
    listEl.innerHTML = "";
...
function updateStyleBtns() {
    document
        ...
        .forEach((b) => b.classList.toggle("active", parseInt(b.dataset.style) === tempStyle));
}
...
document
    ...
    .addEventListener("click", () => document.getElementById("delete-all-modal").classList.add("open"));
document
    .getElementById("cancel-delete-all")
    .addEventListener("click", () => document.getElementById("delete-all-modal").classList.remove("open"));
...
document
    ...
    .addEventListener("click", () => document.getElementById("import-overlay").classList.remove("open"));
document
