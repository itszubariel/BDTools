...
function buildCompBlock(comp, idx) {
    ...
    const typeSpecificNum = components.filter((c, i) => c.type === comp.type && i <= idx).length;
    ...
    else if (comp.type === "userselect" ||
        comp.type === "roleselect" ||
        comp.type === "mentionableselect") {
        const typeName = comp.type === "userselect"
            ? "User Select"
            : comp.type === "roleselect"
                ? "Role Select"
                : "Mentionable Select";
    ...
    d.querySelector("[data-remove]").addEventListener("click", () => {
        ...
        const typeSpecificNum = components.filter((c, i) => c.type === comp.type && i <= compIndex).length;
    ...
    if (comp.type === "stringselect") {
        ...
        if (comp.options && Array.isArray(comp.options)) {
            comp.options.forEach((optData, idx) => {
                const optCard = createStringSelectOption(comp, idx, optData, optionsContainer);
                optionsContainer.appendChild(optCard);
            });
        }
        addOptBtn.addEventListener("click", () => {
            const currentOptionCount = optionsContainer.querySelectorAll(".stringselect-option").length;
            const optCard = createStringSelectOption(comp, currentOptionCount, {}, optionsContainer);
            optionsContainer.appendChild(optCard);
            saveCV2State();
    ...
    d.querySelectorAll("[data-field]").forEach((el) => {
        ...
        el.addEventListener(ev, () => {
            ...
            if ((comp.type === "container" && field === "id") ||
    ...
    d.querySelectorAll("[data-f]").forEach((el) => {
        ...
        const isIdField = ["id", "name", "galleryId", "rowId"].includes(field) &&
            [
                "container",
                "section",
                "mediagallery",
...
function createStringSelectOption(parentComp, optIdx, optData = {}, optionsContainer) {
    ...
    const getCurrentPosition = () => {
        if (!optionsContainer)
            return optIdx + 1;
        const allOptions = Array.from(optionsContainer.querySelectorAll(".stringselect-option"));
    ...
    document.querySelectorAll("#components-root .comp-block").forEach((block) => {
        ...
        allSelects.forEach((sel) => {
            ...
            if (comp.type === "textdisplay" && field === "container") {
                ...
                newHtml = buildRequiredSelect(field, current, [
                    {
                        label: "Sections",
...
document
    ...
    .addEventListener("click", () => {
        syncAllFields();
        const num = components.filter((c) => c.type === "mediagalleryitem").length + 1;
        components.push({
            uid: uid(),
...
document
    ...
    .addEventListener("click", () => {
        syncAllFields();
        const num = components.filter((c) => c.type === "mentionableselect").length + 1;
        components.push({
            uid: uid(),
...
function makeDraggable(el) {
    ...
    handle.addEventListener("dragend", () => {
        ...
        components.sort((a, b) => newOrder.indexOf(a.uid) - newOrder.indexOf(b.uid));
...
function renderCompPreview(comp) {
    ...
    if (comp.type === "section") {
        ...
        const inner = textItems || accessory
...
function generateCode() {
    ...
    components.forEach((comp) => {
        if (comp.type === "container") {
            ...
            if (id)
                allParentIds.push({ id, type: "Container" });
        }
        if (comp.type === "section") {
            const name = (comp.fields.name || "").trim();
            sectionNames.push(name);
            if (name)
                allParentIds.push({ id: name, type: "Section" });
        }
        if (comp.type === "mediagallery") {
            const gid = (comp.fields.galleryId || "").trim();
            galleryIds.push(gid);
            if (gid)
                allParentIds.push({ id: gid, type: "Media Gallery" });
        }
        if (comp.type === "actionrow") {
            const rid = (comp.fields.rowId || "").trim();
            actionRowIds.push(rid);
            if (rid)
                allParentIds.push({ id: rid, type: "Action Row" });
        }
    });
    ...
    components.forEach((comp) => {
        ...
        if (comp.type === "thumbnail") {
            ...
            if (!url)
                errors.push("A Thumbnail is missing its URL");
            else if (!isUrl(url))
                errors.push(`Thumbnail: invalid URL "${url}"`);
            if (!sec)
                errors.push("A Thumbnail is missing its Section Name");
        }
        if (comp.type === "mediagallery") {
        ...
        if (comp.type === "mediagalleryitem") {
            ...
            const gid = (comp.fields.galleryId || "").trim();
            if (!url)
                errors.push("A Media Item is missing its URL");
            else if (!isUrl(url))
                errors.push(`Media Item: invalid URL "${url}"`);
            if (!gid)
                errors.push("A Media Item is missing its Gallery ID");
        ...
        if (comp.type === "buttoncv2") {
            ...
            if (!id)
                errors.push("A Button CV2 is missing its ID");
            if (!label && !emoji)
                errors.push(`Button '${id || "unnamed"}': must have a label or emoji`);
            if (!(comp.fields.actionRowOrSection || "").trim())
                errors.push(`Button '${id || "unnamed"}': Action Row / Section is required`);
        }
        if (comp.type === "userselect" ||
            comp.type === "roleselect" ||
            comp.type === "mentionableselect") {
            const label = comp.type === "userselect"
                ? "User Select"
                : comp.type === "roleselect"
                    ? "Role Select"
                    : "Mentionable Select";
            const id = (comp.fields.id || "").trim();
            if (!id)
                errors.push(`A ${label} is missing its ID`);
        ...
        if (comp.type === "stringselect") {
            ...
            if (!id) {
                ...
                if (!comp.options || comp.options.length === 0) {
                    ...
                    comp.options.forEach((opt, idx) => {
                        ...
                        if (!label || !value) {
                            errors.push(`String Select '${id}' Option ${idx + 1}: label and value required`);
                        }
                        if (label && label.length > 100) {
                            errors.push(`String Select '${id}' Option ${idx + 1}: label max 100 characters`);
                        }
                        if (value && value.length > 100) {
                            errors.push(`String Select '${id}' Option ${idx + 1}: value max 100 characters`);
                        }
                        if (description && description.length > 100) {
                            errors.push(`String Select '${id}' Option ${idx + 1}: description max 100 characters`);
                        }
                        // Check for duplicate values
                        if (value && optionValues.includes(value)) {
                            errors.push(`String Select '${id}': duplicate option value '${value}'`);
                        }
                        else if (value) {
            ...
            if (!(comp.fields.actionRowId || "").trim())
                errors.push(`String Select '${id || "unnamed"}': Action Row ID is required`);
        }
    });
    ...
    components.forEach((comp) => {
        ...
        if (!name)
            return;
        const hasText = components.some((c) => c.type === "textdisplay" && (c.fields.container || "").trim() === name);
        const hasAccessory = components.some((c) => (c.type === "thumbnail" &&
        ...
        if (!hasAccessory)
            errors.push(`Section "${name}": needs an accessory (Thumbnail or Button CV2)`);
        const total = components.filter((c) => (c.type === "textdisplay" &&
            (c.fields.container || "").trim() === name) ||
            (c.type === "thumbnail" && (c.fields.container || "").trim() === name)).length;
        if (total > 3)
    ...
    components.forEach((comp) => {
        ...
        const buttons = components.filter((c) => c.type === "buttoncv2" &&
            (c.fields.actionRowOrSection || "").trim() === rid);
        const selects = components.filter((c) => selTypes.includes(c.type) &&
            (c.fields.actionRowId || "").trim() === rid);
        if (buttons.length === 0 && selects.length === 0)
            errors.push(`Action Row "${rid}": must contain at least one button or select menu`);
        if (selects.length > 1)
            errors.push(`Action Row "${rid}": can only contain one select menu`);
        if (buttons.length > 0 && selects.length > 0)
            errors.push(`Action Row "${rid}": cannot mix buttons and select menus in the same row`);
        if (buttons.length > 5)
            errors.push(`Action Row "${rid}": maximum 5 buttons per row (has ${buttons.length})`);
    });
    ...
    function generateComponent(comp) {
        ...
        if (comp.type === "textdisplay") {
            ...
            if (target) {
                ...
                const dep = components.find((c) => (c.type === "container" || c.type === "section") &&
                    (c.fields.id || c.fields.name || "").trim() === target);
                if (dep)
                    generateComponent(dep);
            }
        }
        else if (comp.type === "separator") {
            const cid = (comp.fields.container || "").trim();
            if (cid) {
                const dep = components.find((c) => c.type === "container" && (c.fields.id || "").trim() === cid);
                if (dep)
                    generateComponent(dep);
            }
        }
            ...
            if (sectionName) {
                const dep = components.find((c) => c.type === "section" &&
            ...
            const gid = (comp.fields.galleryId || "").trim();
            if (gid) {
                const dep = components.find((c) => c.type === "mediagallery" &&
            ...
            if (target) {
                const dep = components.find((c) => (c.type === "actionrow" &&
            ...
            if (rowId) {
                const dep = components.find((c) => c.type === "actionrow" && (c.fields.rowId || "").trim() === rowId);
                if (dep)
                    generateComponent(dep);
            }
        }
        ...
        if (comp.type === "container") {
            ...
            const disabled = comp.fields.disabled ? "true" : "";
            const rowId = comp.fields.actionRowId.trim();
            const funcName = comp.type === "userselect"
                ? "$addUserSelect"
                : comp.type === "roleselect"
                    ? "$addRoleSelect"
                    : "$addMentionableSelect";
            const params = trimParams([id, placeholder, min, max, disabled, rowId], 6);
            ...
            if (comp.options && Array.isArray(comp.options)) {
                comp.options.forEach((opt) => {
                    ...
                    const optParams = trimParams([label, value, description, emoji, defaultVal, selectMenuID], 6);
