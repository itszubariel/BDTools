...
function updatePreview() {
    ...
    const botAvatarURL = "https://scrobbler.netlify.app/assets/images/scrobbler_logo.png";
    const botName = "scrobbler";
    const now = new Date();
    const dateStr = "Today at " +
        now.toLocaleTimeString("en-US", {
            hour: "numeric",
            minute: "2-digit",
        });
    const hasContent = author || title || description || footer || image || thumbnail;
...
["channelID-input", "content-input"].forEach((id) => {
    const el = document.getElementById(id);
    if (el)
        el.addEventListener("input", sebSaveState);
});
...
function renderPresetList(presets) {
    ...
    presets
        ...
        .forEach((preset) => {
            ...
            const fieldCount = Object.values(preset.data).filter((v) => v && v !== false).length;
          ...
          `;
            item
                ...
                .addEventListener("click", () => {
                    ...
                    SEB_FIELD_IDS.forEach((id) => {
                        const el = document.getElementById(id);
                        if (!el || preset.data[id] === undefined)
                            return;
                        if (el.type === "checkbox")
                            el.checked = preset.data[id];
                        else
