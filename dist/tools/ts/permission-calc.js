...
const VAL_TO_NAME = {};
GROUPS.forEach((g) => g.perms.forEach((p) => (VAL_TO_NAME[p.value.toString()] = p.name)));
let bits = 0n;
function buildSections() {
    ...
    GROUPS.forEach((group, gi) => {
        ...
        hdr.addEventListener("mouseover", () => (hdr.style.background = "var(--bg4)"));
        hdr.addEventListener("mouseout", () => (hdr.style.background = ""));
        hdr.addEventListener("click", (e) => {
            if (e.target.closest(".sect-btn"))
                return;
...
function recalc() {
    ...
    list.innerHTML =
        checked.length === 0
            ...
            : checked
                .map((cb) => `<div style="display:flex;align-items:center;justify-content:space-between;gap:8px;background:var(--bg);border-radius:8px;padding:6px 12px;border:1px solid var(--border);">
...
function filterPerms(q) {
    ...
    document.querySelectorAll("#sections>div").forEach((sec) => {
        const any = [...sec.querySelectorAll(".perm-row")].some((r) => r.style.display !== "none");
